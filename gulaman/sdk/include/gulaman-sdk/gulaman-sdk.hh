...
public:
  virtual auto event_select_provider() noexcept -> void = 0;
  virtual auto event_unselect_provider() noexcept -> void = 0;
};

class IG_Provider : public IG_Base {
public:
  gulaman_set_interface_version(0, 1, 0);

  virtual auto transact_start() noexcept -> bool = 0;
  virtual auto transact_end() noexcept -> bool = 0;

  virtual auto target_clear() noexcept -> bool = 0;
  virtual auto target_add(const char * displaytext, unsigned int uid) noexcept -> bool = 0;
  virtual auto target_remove(unsigned int uid) noexcept -> bool = 0;
  virtual auto target_edit_dt(unsigned int uid, const char * newdisplaytext) noexcept -> bool = 0;
};
...
public:
  ...
  virtual auto register_provider(const char * name, const char * description, IP_Provider * implementation) noexcept -> IG_Provider* = 0;
};
